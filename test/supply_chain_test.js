...
contract("ProductManagement", accounts => {
  ...
  it("should create a car using 6 parts and store it", async () => {
    ...
    for (let i = 0; i < serial_numbers.length; i++) {
      result = await contract.buildPart(
        serial_numbers[i],
        part_types[i],
        creation_date,
        { from: accounts[0] }
      ...
      part_array.push(
        web3.utils.soliditySha3(
          accounts[0],
          web3.utils.fromAscii(serial_numbers[i]),
          web3.utils.fromAscii(part_types[i]),
          web3.utils.fromAscii(creation_date)
        )
      );
    }
    ...
    result = await contract.buildProduct(
      serial_prod,
      product_type,
      creation_date,
      part_array,
    ...
    const p_hash = web3.utils.soliditySha3(
      accounts[0],
      web3.utils.fromAscii(serial_prod),
      web3.utils.fromAscii(product_type),
      web3.utils.fromAscii(creation_date)
    );
...
contract("ChangeOwnership", accounts => {
  ...
  it("should be able to assign ownership to manufacturer when part does not have owner", async () => {
    ...
    result = await pm.buildPart(serial_number, part_type, creation_date, {
      from: accounts[0]
    });
    let p_hash = web3.utils.soliditySha3(
      accounts[0],
      web3.utils.fromAscii(serial_number),
      web3.utils.fromAscii(part_type),
      web3.utils.fromAscii(creation_date)
  ...
  it("should be able to change ownership when owner requests", async () => {
    ...
    result = await contract.changeOwnership(op_type, p_hash, accounts[1], {
      from: accounts[0]
    });
